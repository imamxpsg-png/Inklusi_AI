...
def render_top_dashboard_widgets():
    """
    Mengunci visual widget vital agar tetap berdiri sejajar tegak lurus 
    tepat di sebelah kanan kotak video dokumentasi Anda.
    """
    st.markdown("""
...
def render_sidebar_status(active_keys, client, mode="dokumen"):
    # BINGKAI KOTAK UTUH PEMBACA DOKUMEN (BACKGROUND UNGU)
    if mode == "dokumen":
        st.markdown("""
            <div class='card-dokumen'>
                <h3 style='color: #6B21A8; margin: 0;'>🔊 Modul 2 & 3: Pembaca Dokumen & Rangkuman</h3>
        ...
        bahasa_bacaan = st.selectbox("Pilih Bahasa Output Dokumen Belajar:", ["Bahasa Indonesia", "English (Inggris)", "日本語 (Jepang)", "العربية (Arab)", "한국어 (Korea)"], key="lang_doc_sidebar")
        ...
        if uploaded_file is not None:
            ...
            if extracted_text:
                ...
                if st.button("🔊 MULAI BACAKAN DOKUMEN MATERI INI", use_container_width=True):
                    lang_map = {"Bahasa Indonesia": "id", "English (Inggris)": "en", "日本語 (Jepang)": "ja", "العربية (Arab)": "ar", "한국어 (Korea)": "ko"}
                    st.audio(mod.text_to_speech(extracted_text[:400], f"doc_{lang_map.get(bahasa_bacaan)}.mp3"), autoplay=True)
                    st.success(f"✓ Sukses menyiarkan narasi suara dokumen berkas dalam {bahasa_bacaan}!")
        ...
        st.markdown("</div>", unsafe_allow_html=True) # Penutup bingkai ungu
        ...
        if audio_materi and client:
            ...
            with st.spinner("Gemini AI merangkum materi kuliah panjang..."):
                try:
                    ...
                    res = client.models.generate_content(model='gemini-3.6-flash', contents=[types.Part.from_bytes(data=audio_materi['bytes'], mime_type='audio/wav'), "Buat rangkuman terstruktur bahasa indonesia dengan penanda emosi emoji di setiap poin."])
                    if res.text:
