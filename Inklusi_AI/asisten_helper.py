...
def render_asisten_ai(client, sched, mode="asisten"):
    if mode == "asisten":
        ...
        bar_col1, bar_col2, bar_col3 = st.columns([1, 8, 1])
        ...
        # STT PROCESSOR DENGAN MODEL STABIL GEMINI-2.0-FLASH
            ...
            with st.spinner("Mencerna ucapan verbal Anda..."):
                try:
                    ...
                    res = client.models.generate_content(
                        ...
                        contents=[types.Part.from_bytes(data=audio_ai['bytes'], mime_type='audio/wav'), "Salin ucapan suara tersebut menjadi teks bahasa indonesia yang bersih murni tanpa tambahan penjelasan apapun."]
                    )
                    ai_command_text = res.text.strip()
                    st.session_state.microphone_active = False
                except: 
                    st.error("Gagal mendeteksi gelombang suara.")
            ...
            if client:
                with st.spinner("Google Gemini sedang memproses tanggapan Anda..."):
                    try:
                        ...
                        isi_konten.append(f"Request: '{prompt_final}'. Tanggapi dalam Bahasa Indonesia secara komprehensif.")
                        response = client.models.generate_content(model='gemini-2.0-flash', contents=isi_konten)
